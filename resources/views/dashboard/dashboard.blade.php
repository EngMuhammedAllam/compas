    ...
    <div class="container-fluid">
        ...
        <div class="detections-page">
            <div class="row">
                ...
                <!-- مراقبة درجات الحرارة (جديد) -->
                <div class="col-xl-12">
                    <div class="card custom-card">
                        <div class="card-header justify-content-between">
                            <div class="card-title">مراقبة درجات الحرارة (مباشر)</div>
                            <span class="badge bg-danger-transparent text-danger pulse">بث مباشر</span>
                        </div>
                        <div class="card-body">
                            <div class="row">
                                    ...
                                    <div class="card custom-card border shadow-none mb-3">
                                        <div class="card-body p-3">
                                            <div class="d-flex align-items-center mb-3">
                                                <div class="me-2">
                                                    <span class="avatar avatar-md bg-{{ $sensor->is_active ? 'primary' : 'secondary' }}-transparent">
                                                        <i class="ri-temp-hot-line fs-20"></i>
                                                    </span>
                                                </div>
                                                <div class="flex-fill">
                                                    <p class="fw-semibold mb-0 fs-14">{{ $sensor->name }}</p>
                                                    <span class="text-muted fs-11">{{ $sensor->location }}</span>
                                                </div>
                                                <div class="text-end">
                                                    @php
                                                    $currentReading = $sensor->readings->first();
                                                    $isNormal = $currentReading && $currentReading->temperature < $sensor->max_threshold && $currentReading->temperature > $sensor->min_threshold;
                                                        @endphp
                                                        <h4 class="fw-bold mb-0 text-{{ $isNormal ? 'success' : 'danger' }}">
                                                            {{ $currentReading ? $currentReading->temperature : '--' }}°C
                                                        </h4>
                                                </div>
                                            </div>
                                            <div class="d-flex justify-content-between fs-11 text-muted border-top pt-2">
                                                <span>الحد الأدنى: {{ $sensor->min_threshold }}°C</span>
                                                <span>الحد الأعلى: {{ $sensor->max_threshold }}°C</span>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                ...
                <!-- تنبيهات النظام الطارئة (جديد) -->
                <div class="col-xl-6">
                    <div class="card custom-card">
                        <div class="card-header justify-content-between">
                            <div class="card-title text-danger">تنبيهات درجة الحرارة الأخيرة</div>
                            <span class="badge bg-danger text-white">{{ $recent_alerts->count() }}</span>
                        </div>
                                        ...
                                        <tr>
                                            <td>
                                                <div class="d-flex align-items-center">
                                                    <span class="dot-label {{ $alert->is_resolved ? 'bg-success' : 'bg-danger' }} me-2"></span>
                                                    {{ $alert->sensor->name }}
                                                </div>
                                            </td>
                                            <td class="fw-bold text-danger">{{ $alert->temperature }}°C</td>
                                            <td>
                                                <span class="badge bg-{{ $alert->threshold_type == 'high' ? 'warning' : 'info' }}-transparent">
                                                    {{ $alert->threshold_type == 'high' ? 'ارتفاع' : 'انخفاض' }}
                                                </span>
                                            </td>
                                            <td class="text-muted fs-11">{{ $alert->created_at->diffForHumans() }}</td>
                                        </tr>
                                        @endforeach
                                        @if($recent_alerts->isEmpty())
                                        <tr>
                                            <td colspan="4" class="text-center p-4">لا توجد تنبيهات نشطة</td>
                                        </tr>
                                        @endif
                                    </tbody>
                                </table>
                ...
                <div class="col-xl-12">
                    <div class="card custom-card">
                        <div class="card-header justify-content-between">
                            <div class="card-title">تاريخ درجات الحرارة (آخر 24 ساعة)</div>
                        </div>
                        <div class="card-body">
...
<script>
    ...
    window.addEventListener('load', function() {
        ...
        var tempOptions = {
            series: [
                @foreach($sensorHistory as $sensor)
                {
                    name: @json($sensor->name),
                    data: [
                        @foreach($sensor->readings->reverse() as $reading)
                        {
            ...
            colors: ['#3858f9', '#e74c3c', '#27ae60', '#f1c40f'],
            tooltip: {
                x: {
                    format: 'dd MMM HH:mm'
                }
